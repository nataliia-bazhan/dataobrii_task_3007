...
class TravelSample:
    ...
    def select_all(self, n=0):
        ...
        query = f"select {select_cols[:-2]} from {self.table_name} as t"
        if int(n) > 0:
            query += f" limit {int(n)}"
    ...
    def select_column(self, column_name, n=0):
    ...
    def get_all_columns(self):
        ...
        query = f"""
                select distinct object_names(t) as columnNames from {self.table_name} as t
             """
        all_cols = set()
        result = self.cluster.query(query)
        for row in result:
            all_cols.update(row['columnNames'])
        return all_cols
